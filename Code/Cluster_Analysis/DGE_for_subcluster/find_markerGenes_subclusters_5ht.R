...
##Save genes used for DGE for Enrichment analysis
#write_lines(rownames(obj_No_ptprc_no_adgre1),'../../../Processed_Data/DGE_Analysis_no_ptprc_adgre1_18103_genes.txt')
################################################################################################################################################################
# Find Markers
################################################################################################################################################################
...
perform_analysis <- function(unique_type, seuratObj, output_path,pval_cutoff = FALSE,pval_adj_cutoff = FALSE) {
  ...
  corresponding_6Ho_type <- gsub("5ht", "6ho", unique_type)
  if (corresponding_6Ho_type %in% seuratObj$sub_info) {
  subset_6Ho_cells <- subset(seuratObj, sub_info == corresponding_6Ho_type)
  Idents(seuratObj) <- "sub_info"
  markers_cross_ho_ht <- FindMarkers(seuratObj, ident.1 = unique_type, ident.2 = corresponding_6Ho_type, only.pos = FALSE, min.pct = 0.15, min.diff.pct = 0.15, test.use = "MAST",logfc.threshold = 0.585)
  ...
  # Condition 2: Marker genes for this subcluster vs subclusters in same cell type (basal)
  if ("cell_type" %in% colnames(seuratObj@meta.data)) {
    same_type_cells <- subset(seuratObj, cell_type == unique_type_cellType & orig.ident == "5Ht")
    ...
    if(pval_cutoff == TRUE){
      markers_same_type <- markers_same_type[markers_same_type$p_val < 0.05, ] 
    }else if(pval_adj_cutoff == TRUE){
      markers_same_type<- markers_same_type[markers_same_type$p_val_adj < 0.05, ]
    }
...
unique_types <- unique(combined_data$subcluster[grepl("5Ht", combined_data$subcluster, ignore.case = TRUE)])
